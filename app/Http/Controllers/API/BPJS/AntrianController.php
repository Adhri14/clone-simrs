...
use App\Http\Controllers\API\APIBPJSController as APIBPJSController;
...
class AntrianController extends APIBPJSController
{
    ...
    public function response_decrypt($response, $signature)
    {
        if ($response->failed()) {
            ...
            return $this->sendResponse($response->json('metadata.message'), $data, $response->json('metadata.code'));
        }
    }
