...
@section('title', 'Antrian - Aplikasi Pendaftaran')

@section('content_header')
    <h1>Antrian Loket {{ $request->loket ?? '' }} Lantai {{ $request->lantai ?? '' }} - Aplikasi Pendaftaran</h1>
@stop
...
@section('content')
    <div class="row">
        <div class="col-12">
            <x-adminlte-card title="Filter Data Antrian" theme="secondary" collapsible>
                <form action="{{ route('simrs.antrian.pendaftaran') }}" method="get">
                    <div class="row">
                        <div class="col-4">
                            @php
        ...
        @if (isset($request->loket) && isset($request->lantai) && isset($request->tanggal))
            ...
            <div class="col-3">
                <x-adminlte-small-box title="{{ $antrians->where('taskid', 2)->first()->angkaantrean ?? '0' }}"
                    text="Antrian Saat Ini" theme="primary" class="withLoad" icon="fas fa-sign-in-alt" url=""
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($item->taskid <= 2)
                                        ...
                                        @if ($item->taskid == 2)
                                            {{-- panggil ulang --}}
                                            <x-adminlte-button class="btn-xs withLoad" label="Panggil" theme="primary"
                                                icon="fas fa-volume-down" data-toggle="tooltip" title="Panggil Ulang"
                                                onclick="window.location='{{ route('antrian.panggil_pendaftaran', [$item->kodebooking, $request->loket, $request->lantai]) }}'" />
                                            @if ($item->pasienbaru == 1)
                                            ...
                                            @if ($item->pasienbaru == 2)
                                                <x-adminlte-button class="btn-xs btnDaftarOfflineJKN withLoad"
                                                    label="Daftar" theme="success" icon="fas fa-hand-holding-medical"
                                                    data-toggle="tooltip" title="Daftar" data-id="{{ $item->id }}" />
            ...
            <div class="col-12">
                <x-adminlte-card
                    ...
                    theme="secondary" icon="fas fa-info-circle">
                    @php
                        $heads = ['No', 'Kode', 'Tanggal', 'No RM / NIK', 'Jenis / Pasien', 'No Kartu / Rujukan', 'Poliklinik / Dokter', 'Status', 'Action'];
                        $config['order'] = ['7', 'asc'];
                    @endphp
                    <x-adminlte-datatable id="table8" class="nowrap" :heads="$heads" :config="$config" bordered
                        hoverable compressed>
                        @foreach ($antrians->where('taskid', '>', 2) as $item)
                            <tr>
                                <td>{{ $item->angkaantrean }}</td>
    ...
    <x-adminlte-modal id="modalDaftarOfflineJKN" title="Pendaftaran Pasien Offline JKN" size="xl" theme="success"
